...
public abstract class BaseDescriptorValidator<D extends VersionedDescriptor, DV extends DescriptorVersion> {

    private final Class<? extends Enum<?>> descriptorVersionEnum;
    protected Map<DV, DescriptorValidatorConfigBean<D, DV>> validationConfigMap;
     ...
     */
    protected abstract DV readDescriptorVersion(D descriptor);
     ...
     */
    public void configureValidationConfigMap(DV descriptorVersion,
    ...
    public boolean validateOrThrow(D descriptor) {
        ...
        if (null != descriptor) {
            ...
            final DV descriptorVersion = getDescriptorVersionOrThrow(descriptor);
    ...
    protected D validateDescriptorFormatOrThrow(D descriptor) {
        ...
        final DV descriptorVersion = readDescriptorVersion(descriptor);
     ...
     * @throws InvalidBundleException if the one of the values is not the expected one
     */
    private DV getDescriptorVersionOrThrow(D descriptor) {
        final DV descriptorVersion = readDescriptorVersion(descriptor);
     ...
     * @throws InvalidBundleException if the one of the values is not the expected one
     */
    private void validateNullAndNonNullObjects(DV descriptorVersion,
            D descriptor,
            Map<String, Function<D, Object>> objectsThatMustNOTBeNull,
            Map<String, Function<D, Object>> objectsThatMustBeNull) {
